...
def load():
    ...
    docs1 = plumber_loader.load()

    print(f'문서 내용: {docs1[0].page_content}')
    print(f'문서 길이: {len(docs1)}')
    pprint.pp(f'메타데이터: {docs1[0].metadata}')

    mu_loader = PyMuPDFLoader(file_path)

    docs2 = mu_loader.load()

    print(f'문서 내용: {docs2}')
    print(f'문서 길이: {len(docs2)}')
    pprint.pp(f'메타데이터: {docs2[0].metadata}')
